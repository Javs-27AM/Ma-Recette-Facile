                        ...
                        template += `
                            ...
                            <div class="card-body">
                                ...
                                <div style="position: absolute; bottom: 15px; right: 15px;">
                                    <i class="far fa-heart" title="Favorito" style="cursor: pointer; color: #ff6b6b; font-size: 1.5em;"></i>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(!response.error) {
                        ...
                        template += `
                            <div class="card-recipe" recetaId="${receta.ID_Receta}" style="position: relative;">
                                <img src="${receta.Imagen}" alt="Imagen de ${receta.NombreReceta}" class="card-img">
                                <div class="card-body">
                                    <h3 class="card-title">${receta.NombreReceta}</h3>
                                    <p class="card-text">Tiempo de preparación: ${receta.TiempoPreparacion} minutos</p>
...
if (receta.ingredientes && Array.isArray(receta.ingredientes)) {
    receta.ingredientes.forEach(ingrediente => {
        const cantidad = ingrediente.Cantidad || ''; // Mostrar cantidad si está disponible
        $('#ingredientes-section').append(`
            <div class="ingrediente-row">
                ...
                <input type="text" value="${cantidad}" placeholder="Cantidad" class="form-control" required />
            </div>
        `);
    });
} else {
...
if (receta.instrucciones && Array.isArray(receta.instrucciones)) {
    ...
    $(document).on('click', '.card-recipe', function() {
        ...
        if (target.closest('.receta-edit, .receta-delete, .fa-heart, .fa-share').length) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const modalContent = `
                    ...
                    <img src="${receta.Imagen}" alt="Imagen de ${receta.NombreReceta}" class="modal-img">
                    ...
                    <h4>Instrucciones:</h4>
                    <ol>
                        ${receta.instrucciones.map((instruccion, index) => `<li>Paso ${index + 1}: ${instruccion.Descripcion}</li>`).join('')}
                    </ol>
                `;
